...
const App:React.FC<any>= ():any => {
  ...
  const adminMsgs: message[]= useSelector(adminMsgSelector);
  const clientMsgs: message[] = useSelector(clientMsgSelector);
  ...
  const [adminResponder, setAdminResponder] = useState<string>();
  const [username, setUsername] = useState<string>("");
  const [clientResponder, setClientResponder] = useState<string>("");
  const [clientMessageArray, setClientMessageArray] = useState<message[]>([]);
  const [adminMessageArray, setAdminMessageArray] = useState<message[]>([]);
  ...
  const sendMessage = async (user3:string, isAdmin:string, message:string) => {
    try {
